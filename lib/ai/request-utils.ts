...
if (typeof window !== 'undefined') {
  throw new Error('request-utils.ts should only be imported on the server-side');
}
 ...
 */
export function estimateTotalTokens(systemPrompt: string, messages: any[]): number {
  let totalTokens = 0;
...
export function trimMessages(
  ...
  console.log(`[Token Trimming] Current: ${currentTokens} tokens, Max: ${maxTokens} tokens`);
  
  // Start removing oldest messages (but keep last MIN_MESSAGES_TO_KEEP)
  let trimmedMessages = [...messages];
  let removedCount = 0;
  
  while (currentTokens > maxTokens && trimmedMessages.length > MIN_MESSAGES_TO_KEEP) {
  ...
  console.log(`[Token Trimming] Removed ${removedCount} message(s), New total: ${currentTokens} tokens`);
  ...
  if (currentTokens > maxTokens && trimmedMessages.length > 0) {
    console.warn(`[Token Trimming] Still over budget, truncating message content`);
    ...
    for (let i = 0; i < trimmedMessages.length - 1 && tokensRemoved < tokensToRemove; i++) {
      const message = trimmedMessages[i];
      const messageTokens = estimateMessageTokens(message);
      
      // Calculate target size (remove proportional amount)
      const targetSize = Math.max(100, messageTokens - tokensToRemove + tokensRemoved);
      ...
      if (typeof message.content === 'string') {
        ...
        for (const part of message.parts) {
          if (part.type === 'text' && part.text && part.text.length > targetChars) {
...
export function pickApiKeyFromEnv(): string {
  ...
  if (apiKeysEnv) {
    ...
    if (keys.length > 0) {
      ...
      console.log(`[API Key Rotation] Selected key from GROQ_API_KEYS: ...${maskedKey} (${keys.length} keys available)`);
      return selectedKey;
    }
  }
  ...
  // Random selection for load distribution
  const selectedKey = individualKeys[Math.floor(Math.random() * individualKeys.length)];
  const maskedKey = maskApiKey(selectedKey);
  console.log(`[API Key Rotation] Selected key: ...${maskedKey} (${individualKeys.length} keys available)`);
  return selectedKey;
}
